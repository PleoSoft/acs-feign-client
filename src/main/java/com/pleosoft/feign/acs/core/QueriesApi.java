...
public interface QueriesApi {
	 ...
	 * Gets a list of nodes that match the given search criteria. The search
	 * term is used to look for nodes that match against name, title,
	 * description, full text content or tags. The search term: - must contain a
	 * minimum of 3 alphanumeric characters - allows \"quoted term\" - can
	 * optionally use '*' for wildcard matching By default, file and folder
	 * types will be searched unless a specific type is provided as a query
	 * parameter. By default, the search will be across the repository unless a
	 * specific root node id is provided to start the search from. You can sort
	 * the result list using the **orderBy** parameter. You can specify one or
	 * more of the following fields in the **orderBy** parameter: * name *
	 * modifiedAt * createdAt
	 * </p>
	 *
	 *
	 * @param term
	 *            ("The term to search for.", required = true)
	 * @param rootNodeId
	 *            ("The id of the node to start the search from. Supports the
	 ...
	 *            "100")
	 * @param nodeType
	 *            ("Restrict the returned results to only those of the given
	 ...
	 *         or **skipCount** is invalid, or search **term** too short, or
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2 and newer versions.
	 * Gets a list of people that match the given search criteria. The search
	 * term is used to look for matches against person id, firstname and
	 * lastname. The search term: - must contain a minimum of 2 alphanumeric
	 * characters - can optionally use '*' for wildcard matching within the term
	 * You can sort the result list using the **orderBy** parameter. You can
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2 and newer versions.
	 * Gets a list of sites that match the given search criteria. The search
	 * term is used to look for sites that match against site id, title or
	 * description. The search term: - must contain a minimum of 2 alphanumeric
	 * characters - can optionally use '*' for wildcard matching within the term
	 * The default sort order for the returned list is for sites to be sorted by
	 * ascending id. You can override the default by using the **orderBy**
	 * parameter. You can specify one or more of the following fields in the
	 * **orderBy** parameter: * id * title * description
	 * </p>
	 *
	 *
	 * @param term

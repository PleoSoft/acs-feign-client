...
public interface DownloadsApi {
	 ...
	 * versions. Cancels the creation of a download request. **Note:** The
	 * download node can be deleted using the **DELETE /nodes/{downloadId}**
	 * endpoint By default, if the download node is not deleted it will be
	 * picked up by a cleaner job which removes download nodes older than a
	 * configurable amount of time (default is 1 hour) Information about the
	 * existing progress at the time of cancelling can be retrieved by calling
	 * the **GET /downloads/{downloadId}** endpoint The cancel operation is done
	 * asynchronously.
	 * </p>
	 *
	 *
	 * @param downloadId
	 *            ("The identifier of a download node.",required=true)
	 * @return value = { <br/>
	 *         code = 202, message = "The request to cancel a download was
	 *         accepted"),<br/>
	 *         code = 400, message = "Invalid parameter: **downloadId** is
	 *         invalid, or **downloadId** does not point to a node of download
	 *         type "),<br/>
	 *         code = 401, message = "Authentication failed"),<br/>
	 *         code = 403, message = "Current user does not have permission
	 *         cancel the **downloadId** node"),<br/>
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2.1 and newer
	 * versions. Creates a new download node asynchronously, the content of
	 * which will be the zipped content of the **nodeIds** specified in the JSON
	 * body like this: ```JSON { \"nodeIds\": [
	 * \"c8bb482a-ff3c-4704-a3a3-de1c83ccd84c\",
	 * \"cffa62db-aa01-493d-9594-058bc058eeb1\" ] } ``` **Note:** The content of
	 * the download node can be obtained using the **GET
	 * /nodes/{downloadId}/content** endpoint
	 * </p>
	 *
	 *
	 * @param downloadBodyCreate
	 *            (value = "The nodeIds the content of which will be zipped,
	 *            which zip will be set as the content of our download node."
	 *            ,required=true )
	 * @param fields
	 ...
	 *         code = 400, message = "Invalid parameter: **DownloadBodyCreate**
	 *         is invalid due to incorrect syntax, or **nodeIds** is empty, or
	 *         **nodeIds** contains a duplicate **nodeId** "),<br/>
	 *         code = 401, message = "Authentication failed"),<br/>
	 *         code = 403, message = "Current user does not have permission to
	 *         add a certain **nodeId** to the zip"),<br/>
	 *         code = 200, message = "Unexpected error", response =
	 *         Error.class)<br/>
	 *         }
	 *
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2.1 and newer
	 * versions. Retrieve status information for download node **downloadId**
	 * </p>
	 *
	 *
	 * @param downloadId
	 ...
	 *         code = 401, message = "Authentication failed"),<br/>
	 *         code = 403, message = "Current user does not have permission to
	 *         get information about **downloadId** node"),<br/>
	 *         code = 200, message = "Unexpected error", response =
	 *         Error.class)<br/>
	 *         }
	 *

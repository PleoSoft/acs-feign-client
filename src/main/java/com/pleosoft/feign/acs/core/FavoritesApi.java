...
public interface FavoritesApi {
	 ...
	 *
	 * <p>
	 * Favorite a **site**, **file**, or **folder** in the repository. You can use
	 ...
	 * \"target\": { \"file\": { \"guid\": \"abcde-01234-....\" } } }, { \"target\":
	 * { \"file\": { \"guid\": \"abcde-09863-....\" } } }, ] ``` If you specify a
	 ...
	 * @param favoriteBodyCreate (value = "An object identifying the entity to be
	 *                           favorited. The object consists of a single property
	 *                           which is an object with the name `site`, `file`, or
	 *                           `folder`. The content of that object is the `guid`
	 *                           of the target entity. For example, to favorite a
	 *                           file the following body would be used: ```JSON {
	 *                           \"target\": { \"file\": { \"guid\":
	 *                           \"abcde-01234-....\" } } } ``` " ,required=true )
	 * @param include            ("Returns additional information about favorites,
	 ...
	 *         specified id, or an entity exists with the id of the posted object
	 *         but is not of the same type of the posted object "),<br/>
	 ...
	 *
	 * <p>
	 * **Note:** this endpoint is deprecated as of Alfresco 4.2, and will be removed
	 * in the future. Use `/people/{personId}/favorites` instead. Create a site
	 * favorite for person **personId**. You can use the `-me-` string in place of
	 * `<personId>` to specify the currently authenticated user. **Note:** You can
	 * favorite more than one site by specifying a list of sites in the JSON body
	 * like this: ```JSON [ { \"id\": \"test-site-1\" }, { \"id\": \"test-site-2\" }
	 ...
	 *
	 * @param personId               ("The identifier of a person.",required=true)
	 * @param favoriteSiteBodyCreate (value = "The id of the site to favorite."
	 *                               ,required=true )
	 * @param fields                 ("A list of field names. You can use this
	 *                               parameter to restrict the fields returned
	 *                               within a response if, for example, you want to
	 *                               save on overall bandwidth. The list applies to
	 *                               a returned individual entity or entries within
	 *                               a collection. If the API method also supports
	 *                               the **include** parameter, then the fields
	 *                               specified in the **include** parameter are
	 *                               returned in addition to those specified in the
	 *                               **fields** parameter. ")
	 * @return value = { <br/>
	 *         code = 201, message = "Successful response", response =
	 *         FavoriteSiteEntry.class),<br/>
	 *         code = 400, message = "Invalid parameter: **favoriteSiteBodyCreate**
	 *         is invalid "),<br/>
	 *         code = 401, message = "Authentication failed"),<br/>
	 ...
	 *
	 * <p>
	 * Deletes **favoriteId** as a favorite of person **personId**. You can use the
	 ...
	 *
	 * @param personId   ("The identifier of a person.",required=true)
	 * @param favoriteId ("The identifier of a favorite.",required=true)
	 * @return value = { <br/>
	 *         code = 204, message = "Successful response"),<br/>
	 *         code = 401, message = "Authentication failed"),<br/>
	 *         code = 404, message = "**personId** or **favoriteId** does not exist
	 *         "),<br/>
	 *         code = 200, message = "Unexpected error", response =
	 *         Error.class)<br/>
	 *         }
	 ...
	 * <p>
	 * **Note:** this endpoint is deprecated as of Alfresco 4.2, and will be removed
	 * in the future. Use `/people/{personId}/favorites/{favoriteId}` instead.
	 ...
	 * @param personId   ("The identifier of a person.",required=true)
	 * @param favoriteId ("The identifier of a favorite.",required=true)
	 * @param include    ("Returns additional information about favorites, the
	 *                   following optional fields can be requested: * path (note,
	 *                   this only applies to files and folders) ")
	 ...
	 * <p>
	 * **Note:** this endpoint is deprecated as of Alfresco 4.2, and will be removed
	 * in the future. Use `/people/{personId}/favorites/{favoriteId}` instead. Gets
	 * information on favorite site **siteId** of person **personId**. You can use
	 * the `-me-` string in place of `<personId>` to specify the currently
	 * authenticated user.
	 * </p>
	 *
	 ...
	 * <p>
	 * **Note:** this endpoint is deprecated as of Alfresco 4.2, and will be removed
	 * in the future. Use `/people/{personId}/favorites` instead. Gets a list of a
	 * person's favorite sites. You can use the `-me-` string in place of
	 ...
	 * string in place of `<personId>` to specify the currently authenticated user.
	 * You can use the **where** parameter to restrict the list in the response to
	 * entries of a specific kind. The **where** parameter takes a value. The value
	 * is a single predicate that can include one or more **EXISTS** conditions. The
	 * **EXISTS** condition uses a single operand to limit the list to include
	 * entries that include that one property. The property values are: *
	 * `target/file` * `target/folder` * `target/site` For example, the following
	 * **where** parameter restricts the returned list to the file favorites for a
	 * person: ```SQL (EXISTS(target/file)) ``` You can specify more than one
	 * condition using **OR**. The predicate must be enclosed in parentheses. For
	 * example, the following **where** parameter restricts the returned list to the
	 * file and folder favorites for a person: ```SQL (EXISTS(target/file) OR
	 ...
	 *         user does not have permission to access the favorites of **personId**
	 *         "),<br/>
	 *         code = 200, message = "Unexpected error", response =
	 *         Error.class)<br/>
	 *         }

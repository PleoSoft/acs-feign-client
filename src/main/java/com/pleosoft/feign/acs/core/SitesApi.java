...
public interface SitesApi {
	 ...
	 *
	 *
	 * @param siteId                     ("The identifier of a site.",required=true)
	 * @param inviteeId                  ("The invitee user name.",required=true)
	 * @param siteMembershipApprovalBody (value = "Accepting a request to join,
	 *                                   optionally, allows assignment of a role to
	 *                                   the user. " )
	 * @return value = { <br/>
	 ...
	 *         "),<br/>
	 *         code = 422, message = "Integrity exception or not allowed to approve
	 ...
	 * the site id will be generated from the site title. The site id must be unique
	 * and only contain alphanumeric and/or dash characters. Note: the id of a site
	 * cannot be updated once the site has been created. For example, to create a
	 * public site called \"Marketing\" the following body could be used: ```JSON {
	 * \"title\": \"Marketing\", \"visibility\": \"PUBLIC\" } ``` The creation of
	 * the (surf) configuration files required by Share can be skipped via the
	 * **skipConfiguration** query parameter. **Note:** if skipped then such a site
	 * will **not** work within Share. The addition of the site to the user's site
	 * favorites can be skipped via the **skipAddToFavorites** query parameter. The
	 * creator will be added as a member with Site Manager role. When you create a
	 * site, a container called **documentLibrary** is created for you in the new
	 * site. This container is the root folder for content stored in the site.
	 * </p>
	 *
	 *
	 * @param siteBodyCreate     (value = "The site details" ,required=true )
	 * @param skipConfiguration  ("Flag to indicate whether the Share-specific
	 *                           (surf) configuration files for the site should not
	 *                           be created.", defaultValue = "false")
	 * @param skipAddToFavorites ("Flag to indicate whether the site should not be
	 *                           added to the user's site favorites.", defaultValue
	 *                           = "false")
	 ...
	 *         **description** exceed the maximum length; or **id** contains invalid
	 *         characters; or **siteBodyCreate** invalid "),<br/>
	 *         code = 401, message = "Authentication failed"),<br/>
	 ...
	 *
	 * <p>
	 * Creates a site membership for person **personId** on site **siteId**. You can
	 * set the **role** to one of four types: * SiteConsumer * SiteCollaborator *
	 * SiteContributor * SiteManager **Note:** You can create more than one site
	 * membership by specifying a list of people in the JSON body like this: ```JSON
	 * [ { \"role\": \"SiteConsumer\", \"id\": \"joe\" }, { \"role\":
	 * \"SiteConsumer\", \"id\": \"fred\" } ] ``` If you specify a list as input,
	 * then a paginated list rather than an entry is returned in the response body.
	 * For example: ```JSON { \"list\": { \"pagination\": { \"count\": 2,
	 ...
	 *
	 * @param siteId                   ("The identifier of a site.",required=true)
	 * @param siteMembershipBodyCreate (value = "The person to add and their role"
	 *                                 ,required=true )
	 ...
	 *         code = 400, message = "Invalid parameter: value of **role** or **id**
	 *         is invalid or **siteMembershipBodyCreate** invalid "),<br/>
	 *         code = 401, message = "Authentication failed"),<br/>
	 ...
	 *
	 * <p>
	 * Create a site membership request for yourself on the site with the identifier
	 * of **id**, specified in the JSON body. The result of the request differs
	 * depending on the type of site. * For a **public** site, you join the site
	 * immediately as a SiteConsumer. * For a **moderated** site, your request is
	 * added to the site membership request list. The request waits for approval
	 * from the Site Manager. * You cannot request membership of a **private** site.
	 * Members are invited by the site administrator. You can use the `-me-` string
	 ...
	 * specifying a list of sites in the JSON body like this: ```JSON [ {
	 * \"message\": \"Please can you add me\", \"id\": \"test-site-1\", \"title\":
	 * \"Request for test site 1\", }, { \"message\": \"Please can you add me\",
	 * \"id\": \"test-site-2\", \"title\": \"Request for test site 2\", } ] ``` If
	 ...
	 *
	 *
	 * @param personId                        ("The identifier of a
	 *                                        person.",required=true)
	 * @param siteMembershipRequestBodyCreate (value = "Site membership request
	 *                                        details" ,required=true )
	 * @param fields                          ("A list of field names. You can use
	 *                                        this parameter to restrict the fields
	 *                                        returned within a response if, for
	 *                                        example, you want to save on overall
	 *                                        bandwidth. The list applies to a
	 *                                        returned individual entity or entries
	 *                                        within a collection. If the API method
	 *                                        also supports the **include**
	 *                                        parameter, then the fields specified
	 *                                        in the **include** parameter are
	 *                                        returned in addition to those
	 *                                        specified in the **fields** parameter.
	 *                                        ")
	 * @return value = { <br/>
	 *         code = 201, message = "Successful response", response =
	 *         SiteMembershipRequestEntry.class),<br/>
	 *         code = 400, message = "Invalid parameter: **id** is invalid, or the
	 *         user is already invited, or **siteMembershipRequestBodyCreate**
	 *         invalid "),<br/>
	 *         code = 401, message = "Authentication failed"),<br/>
	 *         code = 404, message = "**personId** or **id** does not exist "),<br/>
	 *         code = 200, message = "Unexpected error", response =
	 *         Error.class)<br/>
	 *         }
	 *
	 ...
	 *
	 * <p>
	 * Deletes person **personId** as a member of site **siteId**. You can use the
	 ...
	 *         code = 404, message = "**siteId** or **personId** does not exist
	 *         "),<br/>
	 *         code = 422, message = "Integrity exception (eg. last site member must
	 *         be a site manager) or not allowed to delete member"),<br/>
	 *         code = 200, message = "Unexpected error", response =
	 *         Error.class)<br/>
	 *         }
	 *
	 ...
	 *
	 * <p>
	 * Gets information for site **siteId**. You can use the **relations** parameter
	 * to include one or more related entities in a single response and so reduce
	 * network traffic. The entity types in Alfresco are organized in a tree
	 * structure. The **sites** entity has two children, **containers** and
	 * **members**. The following relations parameter returns all the container and
	 * member objects related to the site **siteId**: ``` containers,members ```
	 * </p>
	 *
	 *
	 * @param siteId    ("The identifier of a site.",required=true)
	 * @param relations ("Use the relations parameter to include one or more related
	 ...
	 *
	 * @param siteId      ("The identifier of a site.",required=true)
	 * @param containerId ("The unique identifier of a site
	 ...
	 *         code = 401, message = "Authentication failed"),<br/>
	 *         code = 404, message = "**siteId** or **containerId** does not exist
	 *         "),<br/>
	 *         code = 200, message = "Unexpected error", response =
	 *         Error.class)<br/>
	 *         }
	 ...
	 * You can use the `-me-` string in place of `<personId>` to specify the
	 ...
	 *
	 * <p>
	 * Gets the site membership request for site **siteId** for person **personId**,
	 * if one exists. You can use the `-me-` string in place of `<personId>` to
	 * specify the currently authenticated user.
	 * </p>
	 *
	 *
	 ...
	 *
	 * <p>
	 * Get the list of site membership requests the user can action. You can use the
	 * **where** parameter to filter the returned site membership requests by
	 * **siteId**. For example: ``` (siteId=mySite) ``` The **where** parameter can
	 * also be used to filter by ***personId***. For example: ```
	 * where=(personId=person) ``` This may be combined with the siteId filter, as
	 * shown below: ``` where=(siteId=mySite AND personId=person)) ```
	 * </p>
	 *
	 *
	 ...
	 *
	 * <p>
	 * Gets a list of site membership information for person **personId**. You can
	 * use the `-me-` string in place of `<personId>` to specify the currently
	 * authenticated user. You can use the **where** parameter to filter the
	 * returned sites by **visibility** or site **preset**. Example to filter by
	 * **visibility**, use any one of: ``` (visibility='PRIVATE')
	 * (visibility='PUBLIC') (visibility='MODERATED') ``` Example to filter by site
	 * **preset**: ``` (preset='site-dashboard') ``` The default sort order for the
	 * returned list is for sites to be sorted by ascending title. You can override
	 * the default by using the **orderBy** parameter. You can specify one or more
	 * of the following fields in the **orderBy** parameter: * id * title * role
	 * </p>
	 *
	 *
	 * @param personId  ("The identifier of a person.",required=true)
	 ...
	 *
	 * <p>
	 * Gets a list of sites in this repository. You can use the **where** parameter
	 * to filter the returned sites by **visibility** or site **preset**. Example to
	 * filter by **visibility**, use any one of: ``` (visibility='PRIVATE')
	 * (visibility='PUBLIC') (visibility='MODERATED') ``` Example to filter by site
	 * **preset**: ``` (preset='site-dashboard') ``` The default sort order for the
	 * returned list is for sites to be sorted by ascending title. You can override
	 * the default by using the **orderBy** parameter. You can specify one or more
	 * of the following fields in the **orderBy** parameter: * id * title *
	 * description You can use the **relations** parameter to include one or more
	 * related entities in a single response and so reduce network traffic. The
	 * entity types in Alfresco are organized in a tree structure. The **sites**
	 * entity has two children, **containers** and **members**. The following
	 * relations parameter returns all the container and member objects related to
	 * each site: ``` containers,members ```
	 * </p>
	 *
	 *
	 * @param skipCount ("The number of entities that exist in the collection before
	 ...
	 *
	 *
	 * @param siteId                      ("The identifier of a
	 *                                    site.",required=true)
	 * @param inviteeId                   ("The invitee user name.",required=true)
	 * @param siteMembershipRejectionBody (value = "Rejecting a request to join,
	 *                                    optionally, allows the inclusion of
	 *                                    comment. " )
	 * @return value = { <br/>
	 *         code = 200, message = "Successful response"),<br/>
	 *         code = 400, message = "Invalid parameter: value of **siteId** or
	 ...
	 *         code = 404, message = "**siteId** or **inviteeId** does not exist
	 *         "),<br/>
	 *         code = 422, message = "Integrity exception or not allowed to reject
	 *         membership request. "),<br/>
	 *         code = 200, message = "Unexpected error", response =
	 *         Error.class)<br/>
	 *         }
	 ...
	 * (site) admin can update title, description or visibility. Note: the id of a
	 * site cannot be updated once the site has been created.
	 * </p>
	 *
	 *
	 * @param siteId         ("The identifier of a site.",required=true)
	 * @param siteBodyUpdate (value = "The site information to update."
	 *                       ,required=true )
	 * @param fields         ("A list of field names. You can use this parameter to
	 *                       restrict the fields returned within a response if, for
	 ...
	 *         code = 200, message = "Successful response", response =
	 *         SiteEntry.class),<br/>
	 *         code = 400, message = "Invalid parameter: **siteBodyUpdate** invalid
	 *         "),<br/>
	 *         code = 401, message = "Authentication failed"),<br/>
	 *         code = 403, message = "Current user does not have permission to
	 ...
	 *
	 * <p>
	 * Update the membership of person **personId** in site **siteId**. You can use
	 ...
	 * @param siteId                   ("The identifier of a site.",required=true)
	 * @param personId                 ("The identifier of a person.",required=true)
	 * @param siteMembershipBodyUpdate (value = "The persons new role"
	 *                                 ,required=true )
	 * @param fields                   ("A list of field names. You can use this
	 *                                 parameter to restrict the fields returned
	 *                                 within a response if, for example, you want
	 ...
	 *         code = 200, message = "Successful response", response =
	 *         SiteMemberEntry.class),<br/>
	 *         code = 400, message = "Invalid parameter: **role** does not exist or
	 *         **siteMembershipBodyUpdate** invalid "),<br/>
	 *         code = 401, message = "Authentication failed"),<br/>
	 *         code = 404, message = "**siteId** or **personId** does not exist
	 *         "),<br/>
	 *         code = 422, message = "Integrity exception (eg. last site member must
	 *         be a site manager) or not allowed to update member"),<br/>
	 *         code = 200, message = "Unexpected error", response =
	 *         Error.class)<br/>
	 *         }
	 *
	 ...
	 *
	 * <p>
	 * Updates the message for the site membership request to site **siteId** for
	 * person **personId**. You can use the `-me-` string in place of `<personId>`
	 ...
	 * @param siteId                          ("The identifier of a
	 *                                        site.",required=true)
	 * @param siteMembershipRequestBodyUpdate (value = "The new message to display"
	 *                                        ,required=true )
	 * @param fields                          ("A list of field names. You can use
	 *                                        this parameter to restrict the fields
	 *                                        returned within a response if, for

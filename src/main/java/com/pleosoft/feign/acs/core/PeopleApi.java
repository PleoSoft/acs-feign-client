...
public interface PeopleApi {
	 ...
	 * Create a person. If applicable, the given person's login access can also be
	 * optionally disabled. You must have admin rights to create a person. You can
	 * set custom properties when you create a person: ```JSON { \"id\":
	 * \"abeecher\", \"firstName\": \"Alice\", \"lastName\": \"Beecher\", \"email\":
	 * \"abeecher@example.com\", \"password\": \"secret\", \"properties\": {
	 * \"my:property\": \"The value\" } } ``` **Note:** setting properties of type
	 * d:content and d:category are not supported.
	 * </p>
	 *
	 *
	 * @param personBodyCreate (value = "The person details." ,required=true )
	 ...
	 * Deletes the avatar image related to person **personId**. You must be the
	 * person or have admin rights to update a person's avatar. You can use the
	 ...
	 * no related avatar then the **placeholder** query parameter can be optionally
	 * used to request a placeholder image to be returned. You can use the `-me-`
	 ...
	 *                        16:56:34 GMT`. " )
	 * @param placeholder     ("If **true** and there is no avatar for this
	 *                        **personId** then the placeholder image is returned,
	 *                        rather than a 404 response. ", defaultValue = "true")
	 * @return value = { <br/>
	 ...
	 *         code = 400, message = "Invalid parameter: **personId** is not a valid
	 *         format "),<br/>
	 *         code = 401, message = "Authentication failed"),<br/>
	 *         code = 404, message = "**personId** does not exist or avatar does not
	 *         exist (and no placeholder requested) "),<br/>
	 *         code = 200, message = "Unexpected error", response =
	 *         Error.class)<br/>
	 *         }
	 *
	 ...
	 *
	 * <p>
	 * Gets information for the person **personId**. You can use the `-me-` string
	 ...
	 *
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2 and newer versions. List
	 * people. You can use the **include** parameter to return any additional
	 * information. The default sort order for the returned list is for people to be
	 * sorted by ascending id. You can override the default by using the **orderBy**
	 * parameter. You can use any of the following fields to order the results: * id
	 * * firstName * lastName
	 * </p>
	 *
	 *
	 ...
	 * @param include   ("Returns additional information about the person. The
	 ...
	 * Initiates the reset password workflow to send an email with reset password
	 * instruction to the user's registered email. The client is mandatory in the
	 * request body. For example: ```JSON { \"client\": \"myClient\" } ``` **Note:**
	 * The client must be registered before this API can send an email. See [server
	 * documentation]. However, out-of-the-box share is registered as a default
	 * client, so you could pass **share** as the client name: ```JSON { \"client\":
	 * \"share\" } ``` **Note:** No authentication is required to call this
	 * endpoint.
	 * </p>
	 *
	 *
	 * @param personId   ("The identifier of a person.",required=true)
	 * @param clientBody (value = "The client name to send email with app-specific
	 *                   url." ,required=true )
	 * @return value = { <br/>
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2.1 and newer versions.
	 * Resets user's password The password, id and key properties are mandatory in
	 * the request body. For example: ```JSON { \"password\":\"newPassword\",
	 * \"id\":\"activiti$10\", \"key\":\"4dad6d00-0daf-413a-b200-f64af4e12345\" }
	 * ``` **Note:** No authentication is required to call this endpoint.
	 * </p>
	 *
	 *
	 * @param personId          ("The identifier of a person.",required=true)
	 * @param passwordResetBody (value = "The reset password details" ,required=true
	 *                          )
	 * @return value = { <br/>
	 *         code = 202, message = "Successful response or even when no workflow
	 *         instance is found with the given **id** or the workflow instance is
	 *         invalid (already been used or expired) or the given **personId** does
	 *         not match the person's id requesting the password reset or the given
	 *         workflow **key** does not match the recovered key. "),<br/>
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2.2 and newer versions.
	 * Updates the avatar image related to the person **personId**. The request body
	 * should be the binary stream for the avatar image. The content type of the
	 * file should be an image file. This will be used to generate an \"avatar\"
	 * thumbnail rendition. You must be the person or have admin rights to update a
	 * person's avatar. You can use the `-me-` string in place of `<personId>` to
	 ...
	 *         code = 200, message = "Successful response"),<br/>
	 *         code = 400, message = "Invalid parameter: **personId** is not a valid
	 *         format or the avatar cannot be uploaded "),<br/>
	 *         code = 401, message = "Authentication failed"),<br/>
	 *         code = 403, message = "Current user does not have permission to
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2 and newer versions.
	 * Update the given person's details. You can use the `-me-` string in place of
	 * `<personId>` to specify the currently authenticated user. If applicable, the
	 * given person's login access can also be optionally disabled or re-enabled.
	 * You must have admin rights to update a person — unless updating your own
	 * details. If you are changing your password, as a non-admin user, then the
	 * existing password must also be supplied (using the oldPassword field in
	 * addition to the new password value). Admin users cannot be disabled by
	 * setting enabled to false. Non-admin users may not disable themselves. You can
	 * set custom properties when you update a person: ```JSON { \"firstName\":
	 * \"Alice\", \"properties\": { \"my:property\": \"The value\" } } ``` **Note:**
	 * setting properties of type d:content and d:category are not supported.
	 * </p>
	 *
	 *
	 * @param personId         ("The identifier of a person.",required=true)
	 * @param personBodyUpdate (value = "The person details." ,required=true )
	 * @param fields           ("A list of field names. You can use this parameter
	 *                         to restrict the fields returned within a response if,
	 *                         for example, you want to save on overall bandwidth.
	 *                         The list applies to a returned individual entity or
	 ...
	 *         invalid or **personId** is not a valid format or **personBodyUpdate**
	 *         is invalid "),<br/>
	 *         code = 401, message = "Authentication failed"),<br/>
	 *         code = 403, message = "Current user does not have permission to
	 *         update a person"),<br/>

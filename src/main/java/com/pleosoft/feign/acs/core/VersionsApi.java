...
public interface VersionsApi {
	 ...
	 * Delete the version identified by **versionId** and **nodeId*. If the
	 * version is successfully deleted then the content and metadata for that
	 * versioned node will be deleted and will no longer appear in the version
	 * history. This operation cannot be undone. If the most recent version is
	 * deleted the live node will revert to the next most recent version. We
	 * currently do not allow the last version to be deleted. If you wish to
	 * clear the history then you can remove the \"cm:versionable\" aspect (via
	 * update node) which will also disable versioning. In this case, you can
	 * re-enable versioning by adding back the \"cm:versionable\" aspect or
	 * using the version params (majorVersion and comment) on a subsequent file
	 * content update.
	 * </p>
	 *
	 *
	 ...
	 * @param versionId
	 *            ("The identifier of a version, ie. version label, within the
	 ...
	 *         valid format, or exists but does not identify a file, or
	 *         **versionId** is invalid "),<br/>
	 ...
	 *         code = 400, message = "Invalid parameter: **nodeId** is not a
	 *         valid format, or it exists but does not identify a file, or
	 *         **versionId** is invalid "),<br/>
	 *         code = 401, message = "Authentication failed"),<br/>
	 ...
	 *         code = 400, message = "Invalid parameter: **nodeId** is not a
	 *         valid format, or is not a file, or **versionId** is invalid
	 *         "),<br/>
	 *         code = 401, message = "Authentication failed"),<br/>
	 *         code = 403, message = "Current user does not have permission for
	 *         **nodeId**"),<br/>
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2 and newer versions.
	 * Gets the version history as an ordered list of versions for the specified
	 ...
	 *            ("The identifier of a node.",required=true)
	 * @param include
	 *            ("Returns additional information about the version node. The
	 ...
	 *            version history of a node.",required=true)
	 * @param revertBody
	 *            (value = "Optionally, specify a version comment and whether
	 ...
	 *         code = 400, message = "Invalid parameter: **nodeId** is not a
	 *         valid format, or it exists but does not identify a file, or
	 *         **versionId** is invalid, or **revertBody** invalid "),<br/>
	 *         code = 401, message = "Authentication failed"),<br/>
	 *         code = 403, message = "Current user does not have permission to
	 *         revert the versioned node"),<br/>
	 *         code = 404, message = "**nodeId** or **versionId** does not exist
	 *         "),<br/>
	 *         code = 422, message = "Model integrity exception trying to revert
	 *         the node"),<br/>
	 *         code = 200, message = "Unexpected error", response =
	 *         Error.class)<br/>
	 *         }

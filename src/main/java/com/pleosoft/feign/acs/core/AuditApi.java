...
public interface AuditApi {
	 ...
	 * versions. Permanently delete audit entries for an audit application
	 * **auditApplicationId**. The **where** clause must be specified, either
	 * with an inclusive time period or for an inclusive range of ids. The
	 * delete is within the context of the given audit application. For example:
	 ...
	 *            ("The identifier of an audit application.",required=true)
	 * @param where
	 *            ("Audit entries to permanently delete for an audit
	 *            application, given an inclusive time period or range of ids.
	 *            For example: * ```where=(createdAt BETWEEN
	 *            ('2017-06-02T12:13:51.593+01:00' ,
	 *            '2017-06-04T10:05:16.536+01:00')``` * ```where=(id BETWEEN
	 *            ('1234', '4321')``` ", required = true)
	 * @return value = { <br/>
	 *         code = 204, message = "Successful response"),<br/>
	 *         code = 400, message = "Invalid parameter: **auditApplicationId**
	 *         is not a valid format or invalid **where** "),<br/>
	 ...
	 *         code = 404, message = "**auditApplicationId** does not exist
	 *         "),<br/>
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2.2 and newer
	 * versions. Permanently delete a single audit entry **auditEntryId**. You
	 * must have admin rights to delete audit information.
	 * </p>
	 *
	 *
	 * @param auditApplicationId
	 *            ("The identifier of an audit application.",required=true)
	 * @param auditEntryId
	 *            ("The identifier of an audit entry.",required=true)
	 * @return value = { <br/>
	 *         code = 204, message = "Successful response"),<br/>
	 *         code = 400, message = "Invalid parameter: **auditApplicationId**
	 *         or **auditEntryId** is not a valid format "),<br/>
	 *         code = 401, message = "Authentication failed"),<br/>
	 *         code = 403, message = "Current user does not have permission to
	 *         delete audit information"),<br/>
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2.2 and newer
	 * versions. Get status of an audit application **auditApplicationId**. You
	 * must have admin rights to retrieve audit information.
	 * </p>
	 *
	 *
	 * @param auditApplicationId
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2.2 and newer
	 * versions. Gets audit entry **auditEntryId**. You must have admin rights
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2.2 and newer
	 * versions. Gets a list of audit applications in this repository. This list
	 * may include pre-configured audit applications, if enabled, such as: *
	 * alfresco-access * CMISChangeLog * Alfresco Tagging Service * Alfresco
	 * Sync Service (used by Enterprise Cloud Sync) You must have admin rights
	 * to retrieve audit information.
	 * </p>
	 *
	 *
	 * @param skipCount
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2.2 and newer
	 * versions. Gets a list of audit entries for audit application
	 * **auditApplicationId**. You can use the **include** parameter to return
	 * additional **values** information. The list can be filtered by one or
	 * more of: * **createdByUser** person id * **createdAt** inclusive time
	 * period * **id** inclusive range of ids * **valuesKey** audit entry values
	 * contains the exact matching key * **valuesValue** audit entry values
	 * contains the exact matching value The default sort order is **createdAt**
	 * ascending, but you can use an optional **ASC** or **DESC** modifier to
	 * specify an ascending or descending sort order. For example, specifying
	 * ```orderBy=createdAt DESC``` returns audit entries in descending
	 * **createdAt** order. You must have admin rights to retrieve audit
	 * information.
	 * </p>
	 *
	 *
	 ...
	 *            ```where=(createdByUser='jbloggs')``` * ```where=(id BETWEEN
	 *            ('1234', '4321')``` * ```where=(createdAt BETWEEN
	 *            ('2017-06-02T12:13:51.593+01:00' ,
	 ...
	 *            ```where=(valuesKey='/alfresco-access/transaction/action' and
	 *            valuesValue='DELETE')``` ")
	 * @param include
	 *            ("Returns additional information about the audit entry. The
	 ...
	 *         code = 400, message = "Invalid parameter: *auditApplicationId**
	 *         is not a valid format, value of **maxItems** or **skipCount** is
	 *         invalid "),<br/>
	 *         code = 401, message = "Authentication failed"),<br/>
	 *         code = 403, message = "User does not have permission to retrieve
	 *         audit information"),<br/>
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2.2 and newer
	 * versions. Gets a list of audit entries for node **nodeId**. The list can
	 * be filtered by **createdByUser** and for a given inclusive time period.
	 * The default sort order is **createdAt** ascending, but you can use an
	 * optional **ASC** or **DESC** modifier to specify an ascending or
	 * descending sort order. For example, specifying ```orderBy=createdAt
	 * DESC``` returns audit entries in descending **createdAt** order. This
	 * relies on the pre-configured 'alfresco-access' audit application.
	 * </p>
	 *
	 *
	 * @param nodeId
	 ...
	 *         valid format, value of **maxItems** or **skipCount** is invalid
	 *         "),<br/>
	 *         code = 401, message = "Authentication failed"),<br/>
	 *         code = 403, message = "User does not have permission to retrieve
	 *         audit information"),<br/>
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2.2 and newer
	 * versions. Disable or re-enable the audit application
	 * **auditApplicationId**. New audit entries will not be created for a
	 * disabled audit application until it is re-enabled (and system-wide
	 * auditing is also enabled). Note, it is still possible to query &/or
	 * delete any existing audit entries even if auditing is disabled for the
	 * audit application. You must have admin rights to update audit
	 * application.
	 * </p>
	 *
	 *
	 * @param auditApplicationId
	 *            ("The identifier of an audit application.",required=true)
	 * @param auditAppBodyUpdate
	 *            (value = "The audit application to update." ,required=true )
	 * @param fields
	 *            ("A list of field names. You can use this parameter to
	 *            restrict the fields returned within a response if, for
	 *            example, you want to save on overall bandwidth. The list
	 ...
	 *         AuditApp.class),<br/>
	 *         code = 400, message = "Invalid parameter: **auditApplicationId**
	 *         is not a valid format or **auditAppBodyUpdate** is invalid
	 *         "),<br/>
	 *         code = 401, message = "Authentication failed"),<br/>
	 *         code = 403, message = "Current user does not have permission to

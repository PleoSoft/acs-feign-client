...
public interface SharedLinksApi {
	 ...
	 * optional expiry date could be set, so the shared link would become invalid
	 * when the expiry date is reached. For example: ```JSON { \"nodeId\":
	 * \"1ff9da1a-ee2f-4b9c-8c34-3333333333\", \"expiresAt\":
	 * \"2017-03-23T23:00:00.000+0000\" } ``` **Note:** You can create shared links
	 * to more than one file specifying a list of **nodeId**s in the JSON body like
	 * this: ```JSON [ { \"nodeId\": \"1ff9da1a-ee2f-4b9c-8c34-4444444444\" }, {
	 * \"nodeId\": \"1ff9da1a-ee2f-4b9c-8c34-5555555555\" } ] ``` If you specify a
	 ...
	 *
	 *
	 * @param sharedLinkBodyCreate (value = "The nodeId to create a shared link
	 *                             for." ,required=true )
	 * @param include              ("Returns additional information about the shared
	 *                             link, the following optional fields can be
	 *                             requested: * allowableOperations * path ")
	 ...
	 *                             overall bandwidth. The list applies to a returned
	 ...
	 *         format, or does not identify a file, or **sharedLinkBodyCreate**
	 *         invalid, or the specified expiry date is invalid. E.g. the expiry
	 ...
	 *
	 *
	 * @param sharedId ("The identifier of a shared link to a file.",required=true)
	 * @return value = { <br/>
	 *         code = 204, message = "Successful response"),<br/>
	 *         code = 400, message = "Invalid parameter: **sharedId** is not a valid
	 *         format "),<br/>
	 *         code = 401, message = "Authentication failed"),<br/>
	 *         code = 403, message = "Current user does not have permission to
	 *         delete **sharedId**"),<br/>
	 ...
	 * example, to email a shared link with minimum info: ```JSON { \"client\":
	 * \"myClient\", \"recipientEmails\": [\"john.doe@acme.com\",
	 * \"joe.bloggs@acme.com\"] } ``` A plain text message property can be
	 * optionally provided in the request body to customise the sent email. Also, a
	 * locale property can be optionally provided in the request body to send the
	 * emails in a particular language (if the locale is supported by Alfresco). For
	 * example, to email a shared link with a messages and a locale: ```JSON {
	 * \"client\": \"myClient\", \"recipientEmails\": [\"john.doe@acme.com\",
	 * \"joe.bloggs@acme.com\"], \"message\": \"myMessage\", \"locale\":\"en-GB\" }
	 * ``` **Note:** The client must be registered before you can send a shared link
	 * email. See [server documentation]. However, out-of-the-box share is
	 * registered as a default client, so you could pass **share** as the client
	 * name: ```JSON { \"client\": \"share\", \"recipientEmails\":
	 * [\"john.doe@acme.com\"] } ```
	 * </p>
	 *
	 *
	 * @param sharedId            ("The identifier of a shared link to a
	 *                            file.",required=true)
	 * @param sharedLinkBodyEmail (value = "The shared link email to send."
	 *                            ,required=true )
	 * @return value = { <br/>
	 *         code = 202, message = "Successful response"),<br/>
	 *         code = 400, message = "Invalid parameter: **sharedId** is not a valid
	 *         format or **sharedLinkBodyEmail** invalid "),<br/>
	 *         code = 401, message = "Authentication failed"),<br/>
	 *         code = 404, message = "**sharedId** does not exist or **client** is
	 *         not registered "),<br/>
	 *         code = 501, message = "Shared links are disabled for the
	 *         system"),<br/>
	 *         code = 200, message = "Unexpected error", response =
	 ...
	 * minimal information for the file with shared link identifier **sharedId**.
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2 and newer versions. Gets
	 * the content of the file with shared link identifier **sharedId**. **Note:**
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2 and newer versions. Gets
	 * rendition information for the file with shared link identifier **sharedId**.
	 * This API method returns rendition information where the rendition status is
	 * CREATED, which means the rendition is available to view/download. **Note:**
	 * No authentication is required to call this endpoint.
	 * </p>
	 *
	 *
	 * @param sharedId    ("The identifier of a shared link to a
	 *                    file.",required=true)
	 * @param renditionId ("The name of a thumbnail rendition, for example *doclib*,
	 *                    or *pdf*.",required=true)
	 * @return value = { <br/>
	 *         code = 200, message = "Successful response", response =
	 *         RenditionEntry.class),<br/>
	 *         code = 400, message = "Invalid parameter: **sharedId** is not a valid
	 *         format, or **renditionId** is invalid "),<br/>
	 *         code = 404, message = "**sharedId** or **renditionId** does not exist
	 *         (ie. not CREATED) "),<br/>
	 *         code = 501, message = "Shared links are disabled for the
	 *         system"),<br/>
	 *         code = 200, message = "Unexpected error", response =
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2 and newer versions. Gets
	 * the rendition content for file with shared link identifier **sharedId**.
	 * **Note:** No authentication is required to call this endpoint.
	 * </p>
	 *
	 *
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2 and newer versions. Gets
	 * a list of the rendition information for the file with shared link identifier
	 * **sharedId**. This API method returns rendition information, including the
	 * rendition id, for each rendition where the rendition status is CREATED, which
	 * means the rendition is available to view/download. **Note:** No
	 ...
	 * a list of links that the current user has read permission on source node. The
	 * list is ordered in descending modified order. **Note:** The list of links is
	 * eventually consistent so newly created shared links may not appear
	 * immediately.
	 * </p>
	 *
	 *
	 ...
	 *                  "100")
	 * @param where     ("Optionally filter the list by \"sharedByUser\" userid of

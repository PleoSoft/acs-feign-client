...
public interface TrashcanApi {
	 ...
	 *         code = 403, message = "User does not have permission to permanently
	 *         delete the deleted node"),<br/>
	 ...
	 * the rendition information for **renditionId** of file **nodeId**.
	 * </p>
	 *
	 *
	 * @param nodeId      ("The identifier of a node.",required=true)
	 ...
	 *         code = 400, message = "Invalid parameter: **nodeId** is not a valid
	 *         format, or is not a file, or **renditionId** is invalid "),<br/>
	 *         code = 401, message = "Authentication failed"),<br/>
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2 and newer versions. Gets
	 * the rendition content for **renditionId** of file **nodeId**.
	 * </p>
	 *
	 *
	 * @param nodeId          ("The identifier of a node.",required=true)
	 ...
	 * @param placeholder     ("If **true** and there is no rendition for this
	 *                        **nodeId** and **renditionId**, then the placeholder
	 *                        image for the mime type of this rendition is returned,
	 *                        rather than a 404 response. ", defaultValue = "false")
	 * @return value = { <br/>
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2 and newer versions. Gets
	 * the specific deleted node **nodeId**.
	 * </p>
	 *
	 *
	 * @param nodeId  ("The identifier of a node.",required=true)
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2 and newer versions. Gets
	 * the content of the deleted node with identifier **nodeId**.
	 * </p>
	 *
	 *
	 * @param nodeId          ("The identifier of a node.",required=true)
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2 and newer versions. Gets
	 * a list of the rendition information for each rendition of the file
	 * **nodeId**, including the rendition id. Each rendition returned has a
	 * **status**: CREATED means it is available to view or download, NOT_CREATED
	 * means the rendition can be requested. You can use the **where** parameter to
	 * filter the returned renditions by **status**. For example, the following
	 ...
	 *         code = 400, message = "Invalid parameter: **nodeId** is not a valid
	 *         format, is not a file, or **where** is invalid "),<br/>
	 *         code = 401, message = "Authentication failed"),<br/>
	 *         code = 403, message = "Current user does not have permission for
	 *         **nodeId**"),<br/>
	 *         code = 404, message = "**nodeId** does not exist "),<br/>
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2 and newer versions. Gets
	 * a list of deleted nodes for the current user. If the current user is an
	 * administrator deleted nodes for all users will be returned. The list of
	 * deleted nodes will be ordered with the most recently deleted node at the top
	 * of the list.
	 * </p>
	 *
	 *
	 ...
	 *                  isFavorite * isLocked * path * properties * permissions ")
	 * @return value = { <br/>
	 *         code = 200, message = "Successful response", response =
	 ...
	 * to a new location. If the node is successfully restored to its former primary
	 * parent, then only the primary child association will be restored, including
	 * recursively for any primary children. It should be noted that no other
	 * secondary child associations or peer associations will be restored, for any
	 * of the nodes within the primary parent-child hierarchy of restored nodes,
	 * irrespective of whether these associations were to nodes within or outside of
	 * the restored hierarchy. Also, any previously shared link will not be restored
	 * since it is deleted at the time of delete of each node.
	 * </p>
	 *
	 *
	 * @param nodeId                 ("The identifier of a node.",required=true)
	 ...
	 *                               **fields** parameter. ")
	 * @param deletedNodeBodyRestore (value = "The targetParentId if the node is
	 ...
	 *         code = 401, message = "Authentication failed"),<br/>
	 *         code = 403, message = "User does not have permission to restore the
	 *         deleted node or user does not have permission to the target
	 *         node"),<br/>
	 ...
	 *         destination"),<br/>
	 *         code = 422, message = "Model integrity exception trying to restore
	 *         the node"),<br/>
	 *         code = 200, message = "Unexpected error", response =
	 *         Error.class)<br/>
	 *         }

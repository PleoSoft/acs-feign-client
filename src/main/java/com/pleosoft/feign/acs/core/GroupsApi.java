...
public interface GroupsApi {
	 ...
	 * versions. Create a group. The group id must start with \"GROUP\\_\". If
	 * this is omitted it will be added automatically. This format is also
	 * returned when listing groups or group memberships. It should be noted
	 * that the other group-related operations also expect the id to start with
	 * \"GROUP\\_\". If one or more parentIds are specified then the group will
	 * be created and become a member of each of the specified parent groups. If
	 * no parentIds are specified then the group will be created as a root
	 * group. The group will be created in the **APP.DEFAULT** and **AUTH.ALF**
	 * zones. You must have admin rights to create a group.
	 * </p>
	 *
	 *
	 * @param groupBodyCreate
	 *            (value = "The group to create." ,required=true )
	 * @param include
	 *            ("Returns additional information about the group. The
	 ...
	 *         code = 409, message = "**id** clashes with an existing group
	 *         "),<br/>
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2.1 and newer
	 * versions. Create a group membership (for an existing person or group)
	 * within a group **groupId**. If the added group was previously a root
	 * group then it becomes a non-root group since it now has a parent. It is
	 * an error to specify an **id** that does not exist. You must have admin
	 * rights to create a group membership.
	 * </p>
	 *
	 *
	 * @param groupId
	 *            ("The identifier of a group.",required=true)
	 * @param groupMembershipBodyCreate
	 *            (value = "The group membership to add (person or sub-group)."
	 *            ,required=true )
	 * @param fields
	 *            ("A list of field names. You can use this parameter to
	 *            restrict the fields returned within a response if, for
	 ...
	 *         code = 403, message = "User does not have permission to create a
	 *         group membership"),<br/>
	 *         code = 404, message = "**groupId** or *id* (of group or person)
	 *         does not exist "),<br/>
	 *         code = 409, message = "Trying to modify a pre-defined system
	 *         group, such as GROUP_EVERYONE. "),<br/>
	 *         code = 200, message = "Unexpected error", response =
	 *         Error.class)<br/>
	 *         }
	 *
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2.1 and newer
	 * versions. Delete group **groupId**. The option to cascade delete applies
	 * this recursively to any hierarchy of group members. In this case,
	 * removing a group member does not delete the person or sub-group itself.
	 * If a removed sub-group no longer has any parent groups then it becomes a
	 * root group. You must have admin rights to delete a group.
	 * </p>
	 *
	 *
	 * @param groupId
	 *            ("The identifier of a group.",required=true)
	 * @param cascade
	 *            ("If **true** then the delete will be applied in cascade to
	 ...
	 *         code = 409, message = "Trying to delete a pre-defined system
	 *         group, such as GROUP_EVERYONE. "),<br/>
	 *         code = 200, message = "Unexpected error", response =
	 *         Error.class)<br/>
	 *         }
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2.1 and newer
	 * versions. Delete group member **groupMemberId** (person or sub-group)
	 * from group **groupId**. Removing a group member does not delete the
	 ...
	 *            ("The identifier of a group.",required=true)
	 * @param groupMemberId
	 *            ("The identifier of a person or group.",required=true)
	 * @return value = { <br/>
	 *         code = 204, message = "Successful response"),<br/>
	 *         code = 400, message = "Invalid parameter: value of **groupId**,
	 *         or **groupMemberId** is invalid "),<br/>
	 *         code = 401, message = "Authentication failed"),<br/>
	 *         code = 403, message = "User does not have permission to delete a
	 *         group membership"),<br/>
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2.1 and newer
	 * versions. Get details for group **groupId**. You can use the **include**
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2.1 and newer
	 * versions. Gets a list of the group memberships for the group **groupId**.
	 * You can use the **where** parameter to filter the returned groups by
	 * **memberType**. Example to filter by **memberType**, use any one of: ```
	 * (memberType='GROUP') (memberType='PERSON') ``` The default sort order for
	 * the returned list is for group members to be sorted by ascending
	 ...
	 *         code = 401, message = "Authentication failed"),<br/>
	 *         code = 404, message = "**groupId** does not exist "),<br/>
	 *         code = 405, message = "Trying to list all members of a
	 *         pre-defined system group, such as GROUP_EVERYONE "),<br/>
	 *         code = 200, message = "Unexpected error", response =
	 *         Error.class)<br/>
	 *         }
	 *
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2.1 and newer
	 * versions. Gets a list of group membership information for person
	 * **personId**. You can use the `-me-` string in place of `<personId>` to
	 * specify the currently authenticated user. You can use the **include**
	 * parameter to return additional information. You can use the **where**
	 * parameter to filter the returned groups by **isRoot**. For example, the
	 * following **where** clause will return just the root groups: ```
	 * (isRoot=true) ``` The **where** parameter can also be used to filter by
	 * ***zone***. This may be combined with isRoot to narrow a result set even
	 * further. For example, the following where clause will only return groups
	 * belonging to the `MY.ZONE` zone. ``` where=(zones in ('MY.ZONE')) ```
	 * This may be combined with the isRoot filter, as shown below: ```
	 * where=(isRoot=false AND zones in ('MY.ZONE')) ``` ***Note:***
	 * restrictions include * `AND` is the only supported operator when
	 * combining `isRoot` and `zones` filters * Only one zone is supported by
	 * the filter * The quoted zone name must be placed in parenthesis — a 400
	 * error will result if these are omitted. The default sort order for the
	 * returned list is for groups to be sorted by ascending displayName. You
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2.1 and newer
	 * versions. Gets a list of groups. You can use the **include** parameter to
	 * return additional information. You can use the **where** parameter to
	 * filter the returned groups by **isRoot**. For example, the following
	 * **where** clause will return just the root groups: ``` (isRoot=true) ```
	 * The **where** parameter can also be used to filter by ***zone***. This
	 * may be combined with isRoot to narrow a result set even further. For
	 * example, the following where clause will only return groups belonging to
	 * the `MY.ZONE` zone. ``` where=(zones in ('MY.ZONE')) ``` This may be
	 * combined with the isRoot filter, as shown below: ``` where=(isRoot=false
	 * AND zones in ('MY.ZONE')) ``` ***Note:*** restrictions include * `AND` is
	 * the only supported operator when combining `isRoot` and `zones` filters *
	 * Only one zone is supported by the filter * The quoted zone name must be
	 * placed in parenthesis — a 400 error will result if these are omitted. The
	 * default sort order for the returned list is for groups to be sorted by
	 * ascending displayName. You can override the default by using the
	 * **orderBy** parameter. You can specify one of the following fields in the
	 ...
	 *         GroupPaging.class),<br/>
	 *         code = 400, message = "Invalid parameter: value of **maxItems**
	 *         or **skipCount**, or **orderBy**, or **where** is invalid
	 *         "),<br/>
	 *         code = 401, message = "Authentication failed"),<br/>
	 *         code = 200, message = "Unexpected error", response =
	 *         Error.class)<br/>
	 ...
	 * <p>
	 * **Note:** this endpoint is available in Alfresco 5.2.1 and newer
	 * versions. Update details (displayName) for group **groupId**. You must
	 * have admin rights to update a group.
	 * </p>
	 *
	 *
	 * @param groupId
	 *            ("The identifier of a group.",required=true)
	 * @param groupBodyUpdate
	 *            (value = "The group information to update." ,required=true )
	 * @param include
	 *            ("Returns additional information about the group. The
	 *            following optional fields can be requested: * parentIds *
	 *            zones ")
